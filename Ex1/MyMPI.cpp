...
int MyMPI::MPI_Recv(void *buf, int count, MPI_Datatype datatype, int source,
	...
	int result;
	if ( status == ((MPI_Status *) 0) ) { // #define MPI_STATUS_IGNORE ((MPI_Status *) 0)
		MPI_Status localStatus;
		result = ::MPI_Recv(buf, count, datatype, source, tag, comm, &localStatus);
		update(&recv, status2count(&localStatus, datatype), datatype);
	} else {
